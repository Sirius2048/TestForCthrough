...
{
    delete [] this->buf_len;
    this->buf_len = (other.buf_len_arraysize==0) ? nullptr : new float[other.buf_len_arraysize];
    buf_len_arraysize = other.buf_len_arraysize;
    for (unsigned int i=0; i<buf_len_arraysize; i++)
        this->buf_len[i] = other.buf_len[i];
}

void BufferLength::parsimPack(omnetpp::cCommBuffer *b) const
{
    ::omnetpp::cPacket::parsimPack(b);
    b->pack(buf_len_arraysize);
    doParsimArrayPacking(b,this->buf_len,buf_len_arraysize);
}
...
void BufferLength::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    delete [] this->buf_len;
    b->unpack(buf_len_arraysize);
    if (buf_len_arraysize==0) {
        this->buf_len = 0;
    } else {
        this->buf_len = new float[buf_len_arraysize];
        doParsimArrayUnpacking(b,this->buf_len,buf_len_arraysize);
    }
}

void BufferLength::setBuf_lenArraySize(unsigned int size)
{
    float *buf_len2 = (size==0) ? nullptr : new float[size];
    unsigned int sz = buf_len_arraysize < size ? buf_len_arraysize : size;
    for (unsigned int i=0; i<sz; i++)
        buf_len2[i] = this->buf_len[i];
    for (unsigned int i=sz; i<size; i++)
        buf_len2[i] = 0;
    buf_len_arraysize = size;
    delete [] this->buf_len;
    this->buf_len = buf_len2;
}

unsigned int BufferLength::getBuf_lenArraySize() const
{
    return buf_len_arraysize;
}

float BufferLength::getBuf_len(unsigned int k) const
{
    if (k>=buf_len_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", buf_len_arraysize, k);
    return this->buf_len[k];
}

void BufferLength::setBuf_len(unsigned int k, float buf_len)
{
    if (k>=buf_len_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", buf_len_arraysize, k);
    this->buf_len[k] = buf_len;
}
...
int BufferLengthDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0]=='b' && strcmp(fieldName, "buf_len")==0) return base+0;
...
int BufferLengthDescriptor::getFieldArraySize(void *object, int field) const
{
    ...
    switch (field) {
        case 0: return pp->getBuf_lenArraySize();
        default: return 0;
    }
}
...
std::string BufferLengthDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    BufferLength *pp = (BufferLength *)object; (void)pp;
    switch (field) {
        case 0: return double2string(pp->getBuf_len(i));
        default: return "";
    }
}
...
bool BufferLengthDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    BufferLength *pp = (BufferLength *)object; (void)pp;
    switch (field) {
        case 0: pp->setBuf_len(i,string2double(value)); return true;
